...
class OccupancyTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 use_decoder=True,
                 **kwargs):
        super(OccupancyTransformer, self).__init__(**kwargs)
        ...
        if prev_bev is not None:
            if use_3d_conv:
                if prev_bev.shape[1] == bev_h * bev_w * bev_z:
                    prev_bev = prev_bev.reshape(bs, bev_z, bev_h, bev_w, -1).reshape( \
                        bs*bev_z, bev_h*bev_w, -1).permute(1, 0, 2)
            else:
                if prev_bev.shape[1] == bev_h * bev_w:
                    prev_bev = prev_bev.permute(1, 0, 2)
            if self.rotate_prev_bev:
                for i in range(prev_bev.shape[1]):
            ...
            if use_3d_conv:
                prev_bev = prev_bev.reshape(bev_h, bev_w, bs, bev_z, -1).permute(3, 0, 1, 2, 4).reshape(bev_z*bev_h*bev_w, bs, -1)
        
        # add can bus signals
        can_bus = bev_queries.new_tensor(np.array([each['can_bus'] for each in kwargs['img_metas']]))
