...
class BEVOccHead(nn.Module): #DETRHead
    ...
    def loss(self,
        ...
        vertices_gt, triangles = mcubes.marching_cubes(gt_occ[0].detach().cpu().numpy(), 0.5)
        vertices_pred, triangles = mcubes.marching_cubes(preds_dicts['occ_preds'][0].detach().cpu().numpy(), 0)
        vertices_pred1, triangles = mcubes.marching_cubes(torch.sigmoid(preds_dicts['occ_preds'][0]).detach().cpu().numpy(), 0.8)
        vertices_pred2, triangles = mcubes.marching_cubes(torch.sigmoid(preds_dicts['occ_preds'][0]).detach().cpu().numpy(), 0.95)

        gt_ = (torch.sigmoid(preds_dicts['occ_preds'][0,0])).type(torch.float).detach().cpu().numpy()
        ...
        vertices_pred2 = vv[gt_ >= 0.3]
        #vv[:, 0] = (vv[:, 0] + 0.5) * 50 /  300  - 25
        #vv[:, 1] = (vv[:, 1] + 0.5) * 50 /  300  - 25
        #vv[:, 2] = (vv[:, 2] + 0.5) * 8 /  48  - 5
        ...
        ply_list = os.listdir('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/')
        #if len(ply_list) <= 40 and not (os.path.exists('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_gt.ply'.format(idx))):
        if loss_dict['loss_occ_{}'.format(3)] < 0.3:
            print('!!!!!!!!!!!!!!!!!!!!')
            os.makedirs('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), exist_ok=True)
            for cam_id, cam_path in enumerate(img_metas[0]['filename']):
                os.system('cp {}  /mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/{}.jpg'.format(cam_path, img_metas[0]['scene_token'], img_metas[0]['lidar_token'], cam_id))
            os.system('cp /mnt/cfs/algorithm/linqing.zhao/BEVFormer{} /mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/lidar.npy'.format(img_metas[0]['pts_filename'][1:], img_metas[0]['scene_token'], img_metas[0]['lidar_token']))

            pcd = o3d.geometry.PointCloud()

            color_gt = gt_occ_ori[0].clone()
            np.save('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/gt.npy'.format(img_metas[0]['scene_token'],
                img_metas[0]['lidar_token']), color_gt.detach().cpu().numpy())
            color_gt = color_gt[(color_gt > 0) * (color_gt < 200)].detach().cpu().numpy()
            color_gt = color_map[color_gt] / 255.0

            gt_ = gt_occ_ori[0].type(torch.float).detach().cpu().numpy()
            x = np.linspace(0, gt_.shape[0] - 1, gt_.shape[0])
            y = np.linspace(0, gt_.shape[1] - 1, gt_.shape[1])
            z = np.linspace(0, gt_.shape[2] - 1, gt_.shape[2])
            X, Y, Z = np.meshgrid(x, y, z,  indexing='ij')
            vv = np.stack([X, Y, Z], axis=-1)
            gt_vv = vv[(gt_ > 0) * (gt_ < 255)]

            pcd.points = o3d.utility.Vector3dVector(gt_vv)
            pcd.colors = o3d.utility.Vector3dVector(color_gt)
            o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/gt.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), pcd)

            #for cam_id, cam_path in enumerate(img_metas[0]['filename']):
            #    os.system('cp {}  /mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/imgs/{}_{}.jpg'.format(cam_path, idx, cam_id))

            score, color = torch.max(torch.softmax(preds_dicts['occ_preds'][-1][0], dim=0), dim=0)
            _, color_ground = torch.max(torch.softmax(preds_dicts['ground_preds'][-1][0], dim=0), dim=0)
            ...
            np.save('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred.npy'.format(img_metas[0]['scene_token'],
                img_metas[0]['lidar_token']), color_ground.detach().cpu().numpy())
            color = color_ground
            gt_ = color.clone()

            color = color[color > 0].detach().cpu().numpy()
            color = color_map[color] / 255.0

            gt_ = gt_.type(torch.float).detach().cpu().numpy()
            x = np.linspace(0, gt_.shape[0] - 1, gt_.shape[0])
            y = np.linspace(0, gt_.shape[1] - 1, gt_.shape[1])
            z = np.linspace(0, gt_.shape[2] - 1, gt_.shape[2])
            X, Y, Z = np.meshgrid(x, y, z,  indexing='ij')
            ...
            pcd.points = o3d.utility.Vector3dVector(pred_vv)
            pcd.colors = o3d.utility.Vector3dVector(color)
            o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), pcd)


            if self.pred_ground:
                color_gt = gt_ground[0].clone()
                color_gt = color_gt[(color_gt > 0) * (color_gt < 200)].detach().cpu().numpy()
                color_gt = color_map[color_gt] / 255.0

                gt_ = gt_ground[0].type(torch.float).detach().cpu().numpy()
                x = np.linspace(0, gt_.shape[0] - 1, gt_.shape[0])
                y = np.linspace(0, gt_.shape[1] - 1, gt_.shape[1])
                z = np.linspace(0, gt_.shape[2] - 1, gt_.shape[2])
                X, Y, Z = np.meshgrid(x, y, z,  indexing='ij')
                ...
                pcd.points = o3d.utility.Vector3dVector(gt_vv)
                pcd.colors = o3d.utility.Vector3dVector(color_gt)
                o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_gt_ground.ply'.format(idx), pcd)
                #for cam_id, cam_path in enumerate(img_metas[0]['filename']):
                #    os.system('cp {}  /mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/imgs/{}_{}.jpg'.format(cam_path, idx, cam_id))

                for index, thr in enumerate([0]):
                    if self.fpn:
                        score, color = torch.max(torch.softmax(preds_dicts['ground_preds'][-1][0], dim=0), dim=0)
                    else:
                        score, color = torch.max(torch.softmax(preds_dicts['ground_preds'][0], dim=0), dim=0)
                    color[score < thr] = 0
                    color = color[color > 0].detach().cpu().numpy()
                    color = color_map[color] / 255.0

                    if self.fpn:
                        score, gt_ = torch.max(torch.softmax(preds_dicts['ground_preds'][-1][0], dim=0), dim=0)
                    else:
                        score, gt_ = torch.max(torch.softmax(preds_dicts['ground_preds'][0], dim=0), dim=0)
                    gt_[score < thr] = 0
                    gt_ = gt_.type(torch.float).detach().cpu().numpy()
                    x = np.linspace(0, gt_.shape[0] - 1, gt_.shape[0])
                    y = np.linspace(0, gt_.shape[1] - 1, gt_.shape[1])
                    ...
                    pcd.points = o3d.utility.Vector3dVector(pred_vv)
                    pcd.colors = o3d.utility.Vector3dVector(color)
                    o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_pred_ground_{}.ply'.format(idx, thr), pcd)



        import open3d as o3d
        idx = np.random.randint(10000)
        if img_metas[0]['lidar_token'] == '45a1e65fe6ae46978559c03cce1c9105' or img_metas[0]['lidar_token'] == '97a785b1ecae4c41b392efcf32884e06' or img_metas[0]['lidar_token'] == 'a8c3ef98266745e9889958167b7b161e': #loss_dict['loss_occ_{}'.format(3)] < 0.3:
            print('!!!!!!!!!!!!!!!!!!!!')
            os.makedirs('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), exist_ok=True)
            for cam_id, cam_path in enumerate(img_metas[0]['filename']):
                os.system('cp {}  /mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/{}.jpg'.format(cam_path, img_metas[0]['scene_token'], img_metas[0]['lidar_token'], cam_id))
            ...
            vertices_gt, triangles_gt = mcubes.marching_cubes((gt_occ_ori[0] > 0).type(torch.float).detach().cpu().numpy(), 0.5)



            pred = torch.sigmoid(preds_dicts['occ_preds'][-1][0,0]).detach().cpu().numpy()
            pred_ground = torch.sigmoid(preds_dicts['ground_preds'][-1][0,0]).detach().cpu().numpy()
            pred = np.max(np.stack([pred, pred_ground], axis=0), axis=0)
            vertices_pred, triangles_pred = mcubes.marching_cubes(pred, 0.5)
            vertices_pred1, triangles_pred1 = mcubes.marching_cubes(pred, 0.3)
            #vertices_pred2, triangles_pred2 = mcubes.marching_cubes(pred, 0.2)
            vertices_pred3, triangles_pred3 = mcubes.marching_cubes(pred, 0.1)

            # vertices_object, triangles_object = mcubes.marching_cubes(torch.sigmoid(preds_dicts['occ_preds'][-1][0,0]).detach().cpu().numpy(), 0.3)
            # vertices_ground, triangles_ground = mcubes.marching_cubes(torch.sigmoid(preds_dicts['ground_preds'][-1][0,0]).detach().cpu().numpy(), 0.3)
            # object_pred = trimesh.Trimesh(vertices_object, triangles_object, process=False)
            # ground_pred = trimesh.Trimesh(vertices_ground, triangles_ground, process=False)
            # #vertices_pred, triangles = mcubes.marching_cubes((torch.argmax(torch.softmax(preds_dicts['occ_preds'][0], dim=0), dim=0) > 0).type(torch.float).detach().cpu().numpy(), 0.5)
            # vertices_pred, triangles = mcubes.marching_cubes(torch.sigmoid(preds_dicts['occ_preds'][-1][0,0]).detach().cpu().numpy(), 0.3)
            # vertices_pred1, triangles = mcubes.marching_cubes(torch.sigmoid(preds_dicts['occ_preds'][-1][0,0]).detach().cpu().numpy(), 0.5)
            # #vertices_pred2, triangles = mcubes.marching_cubes(torch.sigmoid(preds_dicts['occ_preds'][0,0]).detach().cpu().numpy(), 0.5)



            # if self.pred_ground:

            #     ground_gt, triangles = mcubes.marching_cubes((gt_ground[0] > 0).type(torch.float).detach().cpu().numpy(), 0.5)
            #     ground_pred, triangles = mcubes.marching_cubes(torch.sigmoid(preds_dicts['ground_preds'][-1][0,0]).detach().cpu().numpy(), 0.5)
            #     ground_pred1, triangles = mcubes.marching_cubes(torch.sigmoid(preds_dicts['ground_preds'][-1][0,0]).detach().cpu().numpy(), 0.7)
            #     vertices_pred1 = np.concatenate([ground_pred1, vertices_pred1], axis=0)
            #     vertices_pred = np.concatenate([ground_pred, vertices_pred], axis=0)

            # pcd = o3d.geometry.PointCloud()
            # pcd.points = o3d.utility.Vector3dVector(vertices_gt)
            # o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/gt.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), pcd)
            # #pcd.points = o3d.utility.Vector3dVector(vertices_pred1)
            # #o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred1.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), pcd)
            # pcd.points = o3d.utility.Vector3dVector(vertices_pred)
            # o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), pcd)

            mesh_pred = trimesh.Trimesh(vertices_pred, triangles_pred, process=False)
            mesh_pred1 = trimesh.Trimesh(vertices_pred1, triangles_pred1, process=False)
            #mesh_pred2 = trimesh.Trimesh(vertices_pred2, triangles_pred2, process=False)
            mesh_pred3 = trimesh.Trimesh(vertices_pred3, triangles_pred3, process=False)
            mesh_gt = trimesh.Trimesh(vertices_gt, triangles_gt, process=False)
            mesh_pred.export('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), file_type="ply")
            mesh_pred1.export('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred1.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), file_type="ply")
            #mesh_pred2.export('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred2.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), file_type="ply")
            mesh_pred3.export('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred3.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), file_type="ply")
            mesh_gt.export('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/gt.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), file_type="ply")
            #object_pred.export('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/object_pred.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), file_type="ply")
            #ground_pred.export('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/ground_pred.ply'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), file_type="ply")

            np.save('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/ply1/{}_{}/pred.npy'.format(img_metas[0]['scene_token'], img_metas[0]['lidar_token']), pred)
        #print(loss_dict)
        '''
