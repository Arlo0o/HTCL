...
class OccHead(nn.Module):
    ...
    def sample_point_feats(self, points, voxel_feats, img_feats=None, points_uv=None):
        ...
        for batch_index, points_i in enumerate(points):
            ...
            if self.sampling_img_feats: ## True
    ...
    def forward(self, voxel_feats, points=None, img_metas=None, 
        ...
        elif self.supervise_points:  ## True
            # sampling voxel features and apply MLP
            output_points = self.forward_points(points=points, voxel_feats=voxel_feats, img_feats=img_feats, points_uv=points_uv)  ## [20120, 20][20263, 20][19859, 20][20306, 20]
        
        else:
            # propagate voxel predictions to query points
            output_points = []
            for batch_index, points_i in enumerate(points):
                output_points_i = query_points_from_voxels(output_voxels[-1][batch_index].cpu().detach() , 
                                        points_i.detach(), img_metas[batch_index]  )
    ...
    def loss_point_single(self, output_points, target_points, tag):  ## [80548, 20] [80548, 4]
        ...
        # cross entropy
        if self.loss_point_ce_weight > 0: ## True
            loss_dict['loss_point_ce_{}'.format(tag)] = self.point_ce_criterion(output_points, 
                    target_points) * self.loss_point_ce_weight
        
        # lovasz softmax 
        if self.loss_point_lovasz_weight > 0:  ## True
            num_points, num_class = output_points.shape
            output_points = output_points.permute(1, 0).contiguous().view(1, num_class, num_points, 1, 1)  ## [1, 20, 80548, 1, 1]
