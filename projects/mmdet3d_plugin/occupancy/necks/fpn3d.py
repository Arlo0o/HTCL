...
class FPN3D(BaseModule):
    ...
    def __init__(self,
        ...
        if self.num_upsample > 0:
            out_upsample_cfg = dict(type='deconv3d', bias=False)
            self.upsample_convs = nn.ModuleList()
            for i in range(self.num_upsample):
                upsample_conv = nn.Sequential(
                    build_upsample_layer(
                        out_upsample_cfg, in_channels=out_channels,
    ...
    def forward(self, inputs):
        ...
        # extra: upsample the highest level
        if self.num_upsample > 0:
            out_up = outs[0]
            for i, upsample_conv in enumerate(self.upsample_convs):
                out_up = upsample_conv(out_up)
                outs.insert(0, out_up)
