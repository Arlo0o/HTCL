...
from projects.mmdet3d_plugin.datasets.semantic_kitti_dataset import SSCMetrics
import pdb
from sklearn.metrics import confusion_matrix as CM
import time

@DETECTORS.register_module()
class BEVOcc_kitti(MVXTwoStageDetector):
    """BEVFormer.
    Args:
    ...
    def __init__(self,
        ...
        self.lidar_tokens = []
        self.metrics = SSCMetrics(class_names)
    ...
    def forward_test(self, img_metas, img=None, points_occ=None, gt_occ=None, gt_semantic=None, **kwargs):
        ...
        _, output = self.simple_test(
            img_metas, img, prev_bev=None, **kwargs)
        
        # pred_occ: B H W D  (0~20)
        pred_occ = self.post_process(output)
        self.metrics.add_batch(pred_occ.cpu().numpy(), gt_occ.cpu().numpy())
    ...
    def post_process(self, output):
        ...
        if type(pred_occ) == list:
            pred_occ = pred_occ[-1]
        max_score, max_idx = torch.max(pred_occ, dim=1)
