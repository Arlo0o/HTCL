...
class BEVOcc(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        """
        #print('?????????????????????', img_metas[0]['scene_token'],img_metas[0]['lidar_token'])
    ...
    def evaluation(self, pred_occ, points_occ, img_metas, pred_ground=None):
        ...
        for i in range(pred_occ.shape[0]):
            ...
            '''
            if not os.path.exists('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/pred1.ply'):
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(vertices_sum)
                o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/pred1.ply', pcd)
                #pcd.points = o3d.utility.Vector3dVector(vv)
                #o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/pred1.ply', pcd)
                pcd.points = o3d.utility.Vector3dVector(points_occ[i].cpu().numpy())
                o3d.io.write_point_cloud('/mnt/cfs/algorithm/linqing.zhao/BEVFormer/util/gt1.ply', pcd)
            #print(mesh_metrics)
            #pdb.set_trace()
            return mesh_metrics
