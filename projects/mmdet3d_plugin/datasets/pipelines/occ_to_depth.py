...
class CreateDepthFromOccupancy(object):
    ...
    def __call__(self, results):
        ...
        # print('unlabeled = {}, ignored = {}, total = {}'.format(unlabeled_mask.sum(), ignored.sum(), ignored.shape[0]))
...
class CreateDepthFromLiDAR(object):
    ...
    def __call__(self, results):
        ...
        '''
        # norm_points = (lidarseg[:, :3] - self.point_cloud_range[:3]) / (self.point_cloud_range[3:] - self.point_cloud_range[:3])
        # norm_points = (norm_points.float() - 0.5) * 2
        ...
        # out = img_depth.squeeze().data.cpu().numpy()
