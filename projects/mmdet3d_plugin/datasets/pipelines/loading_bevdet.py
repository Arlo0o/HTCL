...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def __init__(self, data_config, is_train=False, using_ego=True, colorjitter=False,
                 ...
                 mmlabnorm=False, load_depth=False, data_root=None, depth_gt_path=None):
        ...
        a100_data_root = '/mnt/cfs2/algorithm/public_data/det3d/nuscenes/origin/'
        if os.path.exists(a100_data_root):
            self.data_root = a100_data_root
        else:
            self.data_root = None
        
        fsd_depth_path = '/mnt/cfs/algorithm/yunpeng.zhang/codes/BEVDepth/data/depth_gt'
        a100_depth_path = '/mnt/cfs2/algorithm/public_data/det3d/nuscenes/origin/depth_gt'
        if os.path.exists(a100_depth_path):
            self.depth_gt_path = a100_depth_path
        else:
            self.depth_gt_path = fsd_depth_path
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam_name in cam_names:
            ...
            if self.data_root is not None:
                filename = filename.replace('./data/nuscenes/', self.data_root)
            ...
            # sensor2lidar_computed = self.get_sensor2lidar_transformation(results['curr'], cam_name, results)
            # dif = (sensor2lidar_computed - sensor2lidar).abs()
            # print(dif.mean(), dif.max())
            ...
            if self.sequential:
                ...
                if not type(results['adjacent']) is list:
                    adj_info = results['adjacent']
                    filename_adj = adj_info['cams'][cam_name]['data_path']
                    img_adjacent = Image.open(filename_adj)
                    img_adjacent = self.img_transform_core(
                        img_adjacent, resize_dims=resize_dims, crop=crop,
                        flip=flip,rotate=rotate)
                    
                    if self.colorjitter and self.is_train:
                        img_adjacent = self.pipeline_colorjitter(img_adjacent)
                    
                    imgs.append(self.normalize_img(img_adjacent, img_norm_cfg=self.img_norm_cfg))
                else:
                    for id in range(len(results['adjacent'])):
                        adj_info = results['adjacent'][id]
                        filename_adj = adj_info['cams'][cam_name]['data_path']
                        img_adjacent = Image.open(filename_adj)
                        img_adjacent = self.img_transform_core(
                            img_adjacent, resize_dims=resize_dims, crop=crop,
        ...
        if self.sequential:
            ...
            sensor2sensors_adj = []
            for cam_name in cam_names:
                adjsensor2keylidar = self.get_sensor2lidar_transformation(results['adjacent'], cam_name, results)
                
                rot = adjsensor2keylidar[:3, :3]
                tran = adjsensor2keylidar[:3, 3]
                rots_adj.append(rot)
                trans_adj.append(tran)
                sensor2sensors_adj.append(adjsensor2keylidar)
...
class LoadAnnotationsBEVDepth():
    def __init__(self, bda_aug_conf, classes,
                 speed_mode='abs_speed', is_train=True):
    ...
    def __call__(self, results):
        ...
        if 'dis' in self.speed_mode:
            time_diff = np.mean([results['curr']['cams'][cam]['timestamp']
                                 for cam in results['curr']['cams']]) - \
                        np.mean([results['adjacent']['cams'][cam]['timestamp']
                                 for cam in results['adjacent']['cams']])
            time_diff = 1e-6 * time_diff
            gt_boxes[:,-2:] = gt_boxes[:,-2:] * time_diff
        
        results['gt_bboxes_3d'] = LiDARInstance3DBoxes(gt_boxes,
